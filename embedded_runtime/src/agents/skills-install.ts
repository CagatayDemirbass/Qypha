...
function buildVerifiedInstallMessage(params: { skillName: string; bins: string[] }): string {
  const bins = Array.from(new Set(params.bins.map((bin) => bin.trim()).filter(Boolean)));
  if (bins.length === 0) {
    return `Installed and verified ${params.skillName}`;
  }
  return `Installed and verified ${params.skillName} (${bins.join(", ")})`;
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  ensureSkillsWatcher({ workspaceDir, config: params.config });
  bumpSkillsSnapshotVersion({
    workspaceDir,
    reason: "manual",
    changedPath: verification.entry.skill.filePath,
  });

  return withWarnings(
    {
      ...installResult,
      message: buildVerifiedInstallMessage({
        skillName: params.skillName,
        bins: spec.bins ?? [],
      }),
    },
