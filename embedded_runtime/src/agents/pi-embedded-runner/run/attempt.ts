...
import { buildEmbeddedCapabilityConfig } from "../../embedded-capability-config.js";
...
export async function runEmbeddedAttempt(
  ...
  embeddedDebugLog(`attempt:start session=${params.sessionId}`);
  ...
  embeddedDebugLog(`attempt:workspace_ready session=${params.sessionId}`);
  ...
  embeddedDebugLog(
    `attempt:sandbox_resolved session=${params.sessionId} enabled=${Boolean(sandbox?.enabled)}`,
  );
  ...
  embeddedDebugLog(`attempt:effective_workspace_ready session=${params.sessionId}`);

  let restoreSkillEnv: (() => void) | undefined;
  try {
    const minimalEmbeddedAttempt = process.env.QYPHA_EMBEDDED_MINIMAL_ATTEMPT === "1";
    const { shouldLoadSkillEntries, skillEntries } = minimalEmbeddedAttempt
    ...
    restoreSkillEnv = minimalEmbeddedAttempt
      ? undefined
    ...
    const skillsPrompt = minimalEmbeddedAttempt
      ? undefined
      : resolveSkillsPromptForRun({
          skillsSnapshot: params.skillsSnapshot,
    ...
    const { bootstrapFiles: hookAdjustedBootstrapFiles, contextFiles } = minimalEmbeddedAttempt
    ...
    const workspaceNotes = hookAdjustedBootstrapFiles.some(
      ...
      : undefined;
    embeddedDebugLog(`attempt:bootstrap_ready session=${params.sessionId}`);
    ...
    embeddedDebugLog(`attempt:pre_tools session=${params.sessionId}`);
    // Check if the model supports native image input
    const modelHasVision = params.model.input?.includes("image") ?? false;
    embeddedDebugLog(`attempt:tools_raw_start session=${params.sessionId}`);
    ...
    embeddedDebugLog(`attempt:tools_raw_done session=${params.sessionId} count=${toolsRaw.length}`);
    ...
    const embeddedCapabilityConfig = buildEmbeddedCapabilityConfig({
      workspaceDir: effectiveWorkspace,
      baseConfig: params.config,
    });
    const bundleMcpRuntime =
      toolsEnabled && !minimalEmbeddedAttempt
        ? await createBundleMcpToolRuntime({
            workspaceDir: effectiveWorkspace,
            cfg: embeddedCapabilityConfig,
            reservedToolNames: [
        ...
        : undefined;
    embeddedDebugLog(
      `attempt:mcp_runtime_done session=${params.sessionId} enabled=${Boolean(bundleMcpRuntime)}`,
    );
    const bundleLspRuntime =
      toolsEnabled && !minimalEmbeddedAttempt
        ...
        : undefined;
    embeddedDebugLog(
      `attempt:lsp_runtime_done session=${params.sessionId} enabled=${Boolean(bundleLspRuntime)}`,
    );
    ...
    const configuredMcpInventory = inspectBundleMcpInventory({
      workspaceDir: effectiveWorkspace,
      cfg: embeddedCapabilityConfig,
    });
    const appendPrompt = buildEmbeddedSystemPrompt({
      ...
      runtimeInfo,
      mcpConfigPath:
        process.env.QYPHA_RUNTIME_CONFIG_PATH?.trim() ||
        process.env.OPENCLAW_CONFIG_PATH?.trim() ||
        undefined,
      configuredMcpServers: configuredMcpInventory.configuredServers,
      mcpServers: bundleMcpRuntime?.servers,
      messageToolHints,
      sandboxInfo,
      tools: effectiveTools,
    ...
    try {
      ...
      embeddedDebugLog(`attempt:create_session_start session=${params.sessionId}`);
      ...
      embeddedDebugLog(`attempt:create_session_done session=${params.sessionId}`);
      ...
      embeddedDebugLog(`attempt:subscribe_start session=${params.sessionId}`);
      ...
      embeddedDebugLog(`attempt:subscribe_done session=${params.sessionId}`);
      ...
      try {
        ...
        try {
          ...
          embeddedDebugLog(`attempt:prompt_start session=${params.sessionId}`);
          if (imageResult.images.length > 0) {
            await abortable(activeSession.prompt(effectivePrompt, { images: imageResult.images }));
          } else {
            await abortable(activeSession.prompt(effectivePrompt));
          }
          embeddedDebugLog(`attempt:prompt_done session=${params.sessionId}`);
        } catch (err) {
        ...
        try {
          ...
          embeddedDebugLog(
            `attempt:compaction_wait_done session=${params.sessionId} timedOut=${Boolean(compactionRetryWait.timedOut)}`,
          );
      ...
      embeddedDebugLog(`attempt:cleanup_start session=${params.sessionId}`);
      ...
      embeddedDebugLog(`attempt:flush_done session=${params.sessionId}`);
      session?.dispose();
      embeddedDebugLog(`attempt:session_disposed session=${params.sessionId}`);
      releaseWsSession(params.sessionId);
      await bundleMcpRuntime?.dispose();
      await bundleLspRuntime?.dispose();
      await sessionLock.release();
      embeddedDebugLog(`attempt:cleanup_done session=${params.sessionId}`);
    }
  } finally {
    restoreSkillEnv?.();
  }
