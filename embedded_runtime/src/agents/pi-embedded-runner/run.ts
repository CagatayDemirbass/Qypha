...
export async function runEmbeddedPiAgent(
  ...
  embeddedDebugLog(
    `run:start session=${params.sessionId} provider=${params.provider ?? ""} model=${params.model ?? ""}`,
  );
  const sessionLane = resolveSessionLane(params.sessionKey?.trim() || params.sessionId);
  const globalLane = resolveGlobalLane(params.lane);
  embeddedDebugLog(
    `run:lanes_resolved session=${params.sessionId} sessionLane=${sessionLane} globalLane=${globalLane}`,
  );
  const enqueueGlobal =
  ...
  embeddedDebugLog(`run:before_session_enqueue session=${params.sessionId}`);
  return enqueueSession(() => {
    embeddedDebugLog(`run:session_enqueue_enter session=${params.sessionId}`);
    return enqueueGlobal(async () => {
      embeddedDebugLog(`run:global_enqueue_enter session=${params.sessionId}`);
      ...
      embeddedDebugLog(`run:plugins_loaded session=${params.sessionId}`);
      ...
      embeddedDebugLog(`run:models_json_start session=${params.sessionId}`);
      await ensureOpenClawModelsJson(params.config, agentDir);
      embeddedDebugLog(`run:models_json_done session=${params.sessionId}`);
      ...
      embeddedDebugLog(`run:hook_runner_ready session=${params.sessionId} hasRunner=${Boolean(hookRunner)}`);
      const hookCtx = {
      ...
      embeddedDebugLog(
        `run:model_resolved session=${params.sessionId} provider=${provider} model=${modelId} ok=${Boolean(model)}`,
      );
      if (!model) {
      ...
      ensureContextEnginesInitialized();
      embeddedDebugLog(`run:context_init_done session=${params.sessionId}`);
      const contextEngine = await resolveContextEngine(params.config);
      embeddedDebugLog(`run:context_resolved session=${params.sessionId}`);
      try {
        ...
        while (true) {
          ...
          embeddedDebugLog(
            `run:attempt_completed session=${params.sessionId} aborted=${Boolean(attempt.aborted)} timedOut=${Boolean(attempt.timedOut)} promptError=${attempt.promptError ? "yes" : "no"}`,
          );
        ...
        embeddedDebugLog(`run:cleanup_start session=${params.sessionId}`);
