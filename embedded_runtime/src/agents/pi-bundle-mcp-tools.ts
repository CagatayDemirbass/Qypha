...
export async function createBundleMcpToolRuntime(params: {
  ...
  if (Object.keys(loaded.mcpServers).length === 0) {
    return { tools: [], servers: [], dispose: async () => {} };
  }
  ...
  const servers: BundleMcpToolRuntime["servers"] = [];
    ...
    for (const [serverName, rawServer] of Object.entries(loaded.mcpServers)) {
      ...
      try {
        ...
        for (const tool of listedTools) {
          ...
          const serverAttribution = `Provided by embedded MCP server "${serverName}" (${describeStdioMcpServerLaunchConfig(launchConfig)}).`;
          tools.push({
            name: tool.name,
            label: tool.title ?? tool.name,
            description: [tool.description?.trim(), serverAttribution].filter(Boolean).join(" "),
        ...
        servers.push({
          serverName,
          toolNames: Array.from(new Set(serverToolNames)).sort((left, right) =>
...
export function inspectBundleMcpInventory(params: {
  workspaceDir: string;
  cfg?: OpenClawConfig;
}): BundleMcpInventory {
  const loaded = loadEmbeddedPiMcpConfig({
  ...
  return {
    configuredServers: Object.keys(loaded.mcpServers)
