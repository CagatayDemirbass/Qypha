...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.QYPHA_RUNTIME_PROFILE?.trim() ?? env.OPENCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".qypha-runtime", `workspace-${profile}`);
  }
  return path.join(home, ".qypha-runtime", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".qypha-runtime";
