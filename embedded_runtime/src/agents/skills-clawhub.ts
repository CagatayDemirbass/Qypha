...
import { bumpSkillsSnapshotVersion, ensureSkillsWatcher } from "./skills/refresh.js";
import { verifyWorkspaceSkill } from "./skills-verify.js";
...
async function restoreFileSnapshot(filePath: string, snapshot: FileSnapshot): Promise<void> {
  ...
  await fs.writeFile(filePath, snapshot.raw, "utf8");
}
...
async function installExtractedSkill(params: {
  ...
  mode: "install" | "update";
  afterPublish?: (targetDir: string) => void | Promise<void>;
  logger?: Logger;
  ...
  const install = await installPackageDir({
    ...
    hasDeps: false,
    depsLogMessage: "",
    afterPublish: params.afterPublish,
  });
  if (!install.ok) {
    return install;
  }
...
async function performClawHubSkillInstall(
  ...
  try {
    ...
    const targetDir = resolveSkillInstallDir(params.workspaceDir, params.slug);
    const lockfilePath = path.join(params.workspaceDir, DOT_DIR, "lock.json");
    const lockfileSnapshot = await captureFileSnapshot(lockfilePath);
      ...
      const install = await withExtractedArchiveRoot({
        ...
        onExtracted: async (rootDir) =>
          await installExtractedSkill({
            ...
            afterPublish: async (installedDir) => {
              const verification = verifyWorkspaceSkill({
                workspaceDir: params.workspaceDir,
                config: params.config,
                expectedBaseDir: installedDir,
              });
              if (!verification.ok) {
              ...
              try {
                await writeClawHubSkillOrigin(installedDir, {
                  version: 1,
                ...
                await restoreFileSnapshot(lockfilePath, lockfileSnapshot).catch(() => undefined);
                throw error;
              }
            },
            logger: params.logger,
