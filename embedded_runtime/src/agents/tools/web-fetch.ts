...
const DEFAULT_PDF_FETCH_MAX_PAGES = 64;
const DEFAULT_PDF_FETCH_MIN_TEXT_CHARS = 200;
const DEFAULT_PDF_FETCH_MAX_PIXELS = 4_000_000;
...
function looksLikePdfUrl(rawUrl: string): boolean {
  ...
  return (
    ...
    trimmed.includes("format=pdf") ||
    trimmed.includes("download=pdf")
  );
}

function looksLikePdfResponse(params: {
  ...
  if (
    ...
    normalizedContentType.includes("application/x-pdf")
  ) {
    return true;
  }
  return looksLikePdfUrl(params.finalUrl) || looksLikePdfUrl(params.url);
}

async function extractPdfForWebFetch(params: {
  response: Response;
  maxBytes: number;
  extractMode: ExtractMode;
}): Promise<{ text: string; warning?: string }> {
  const pdfBuffer = await readResponseWithLimit(params.response, params.maxBytes, {
    onOverflow: ({ maxBytes, res }) =>
      new Error(
        `Web fetch PDF too large from ${res.url || "remote URL"}: payload exceeds ${maxBytes} bytes`,
      ),
  });
  const extracted = await extractPdfContent({
    buffer: pdfBuffer,
    maxPages: DEFAULT_PDF_FETCH_MAX_PAGES,
    maxPixels: DEFAULT_PDF_FETCH_MAX_PIXELS,
    minTextChars: DEFAULT_PDF_FETCH_MIN_TEXT_CHARS,
  });

  const extractedText = extracted.text.trim();
  if (extractedText) {
    return {
      text: params.extractMode === "markdown" ? extractedText : extractedText,
      warning:
        extracted.images.length > 0
          ? "PDF extraction included rendered page images for low-text pages; use pdf for deeper document analysis when needed."
          : undefined,
    };
  }
  ...
  if (extracted.images.length > 0) {
    return {
      text: "PDF detected, but text extraction was limited because the document appears image-heavy. Use the pdf tool for deeper page-aware analysis.",
      warning: "PDF extraction fell back to images; richer analysis should use the pdf tool.",
    };
  }

  return {
    text: "PDF detected, but no readable text could be extracted from the document.",
    warning: "PDF extraction returned no readable text.",
  };
}
...
async function runWebFetch(params: WebFetchRuntimeParams): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    const isPdf = looksLikePdfResponse({
    ...
    if (!isPdf) {
      ...
      responseTruncatedWarning = bodyResult.truncated
    ...
    let extractor = "raw";
    let text = body;
    let extractorWarning: string | undefined;
    if (isPdf) {
      const pdfResult = await extractPdfForWebFetch({
        response: res,
      ...
      extractorWarning = pdfResult.warning;
      try {
        title = decodeURIComponent(new URL(finalUrl).pathname.split("/").pop() ?? "").trim() || undefined;
      } catch {
        title = undefined;
      }
    ...
    const warningParts = [responseTruncatedWarning, extractorWarning].filter(
      (value): value is string => Boolean(value?.trim()),
    );
    const wrappedWarning = wrapWebFetchField(warningParts.join(" ").trim() || undefined);
    const payload = {
