...
import { createFindTool, createGrepTool, createLsTool } from "./tools/grep-find-ls-tool.js";
import {
...
export function createOpenClawCodingTools(options?: {
  ...
  const grepTool = createGrepTool({ workspaceDir: sandboxRoot ?? workspaceRoot });
  const findTool = createFindTool({ workspaceDir: sandboxRoot ?? workspaceRoot });
  const lsTool = createLsTool({ workspaceDir: sandboxRoot ?? workspaceRoot });
