...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".qypha-runtime");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitQyphaHome = process.env.QYPHA_RUNTIME_HOME?.trim();
  if (explicitQyphaHome) {
    return { home, prefix: "$QYPHA_RUNTIME_HOME" };
  }
  const explicitLegacyHome = process.env.OPENCLAW_HOME?.trim();
  if (explicitLegacyHome) {
...
// Configuration root; can be overridden via QYPHA_RUNTIME_STATE_DIR.
