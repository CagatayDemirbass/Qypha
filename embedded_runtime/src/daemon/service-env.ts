...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    QYPHA_RUNTIME_STATE_DIR: sharedEnv.stateDir,
    QYPHA_RUNTIME_CONFIG_PATH: sharedEnv.configPath,
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.QYPHA_RUNTIME_STATE_DIR ?? env.OPENCLAW_STATE_DIR;
  const configPath = env.QYPHA_RUNTIME_CONFIG_PATH ?? env.OPENCLAW_CONFIG_PATH;
