...
export function applyCliProfileEnv(params: {
  ...
  env.QYPHA_RUNTIME_PROFILE = env.QYPHA_RUNTIME_PROFILE ?? profile;
  env.OPENCLAW_PROFILE = env.OPENCLAW_PROFILE ?? profile;

  const stateDir =
    env.QYPHA_RUNTIME_STATE_DIR?.trim() ||
    env.OPENCLAW_STATE_DIR?.trim() ||
    resolveProfileStateDir(profile, env, homedir);
  if (!env.QYPHA_RUNTIME_STATE_DIR?.trim()) {
    env.QYPHA_RUNTIME_STATE_DIR = stateDir;
  }
  if (!env.OPENCLAW_STATE_DIR?.trim()) {
    env.OPENCLAW_STATE_DIR = stateDir;
  }

  const configPath = path.join(stateDir, "qypha-runtime.json");
  if (!env.QYPHA_RUNTIME_CONFIG_PATH?.trim()) {
    env.QYPHA_RUNTIME_CONFIG_PATH = configPath;
  }
