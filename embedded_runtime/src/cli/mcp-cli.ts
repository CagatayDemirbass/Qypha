...
export function registerMcpCli(program: Command) {
  ...
  mcp
    ...
    .action(async (name: string, rawValue: string) => {
      ...
      const verifiedTools = result.verification?.toolNames ?? [];
      const toolSummary =
        verifiedTools.length > 0
          ? ` Verified ${verifiedTools.length} tool(s): ${verifiedTools.slice(0, 8).join(", ")}${verifiedTools.length > 8 ? ", …" : ""}.`
          : "";
      const launchSummary = result.verification?.launchSummary
        ? ` Launch: ${result.verification.launchSummary}.`
        : "";
      defaultRuntime.log(
        `Saved and verified MCP server "${name}" to ${result.path}.${launchSummary}${toolSummary}`,
      );
    });
