...
export const POSIX_OPENCLAW_TMP_DIR = "/tmp/qypha-runtime";
...
export function resolvePreferredOpenClawTmpDir(
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "qypha-runtime" : `qypha-runtime-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[qypha-runtime] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Qypha runtime temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Qypha runtime temp dir: ${fallbackPath}`);
    }
