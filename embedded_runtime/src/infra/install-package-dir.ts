...
export async function installPackageDir(params: {
  ...
  afterPublish?: (installedDir: string) => void | Promise<void>;
  ...
  const fail = async (error: string, cause?: unknown) => {
    ...
    if (installBaseChanged) {
      ...
      if (backupDir) {
        ...
        await fs.rm(canonicalTargetDir, { recursive: true, force: true }).catch(() => undefined);
      }
      if (stageDir) {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".qypha-runtime-install-stage-"));
  ...
  if (params.mode === "update" && (await fileExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".qypha-runtime-install-backups");
  ...
  try {
    await params.afterPublish?.(canonicalTargetDir);
  } catch (err) {
    return await fail(`post-publish verification failed: ${String(err)}`, err);
  }
