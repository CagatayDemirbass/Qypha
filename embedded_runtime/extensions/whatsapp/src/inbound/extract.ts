...
type MessageContentKey = (typeof MESSAGE_CONTENT_KEYS)[number];
type ContactLike = {
  displayName?: string | null;
  vcard?: string | null;
};

function isMessageContentKey(value: unknown): value is MessageContentKey {
  return (
    typeof value === "string" &&
    MESSAGE_CONTENT_KEYS.includes(value as (typeof MESSAGE_CONTENT_KEYS)[number])
  );
}
...
function getMessageContentType(
  ...
  if (typeof getContentType === "function") {
    ...
    if (isMessageContentKey(contentType)) {
      return contentType;
    }
  }
...
function extractContactPlaceholder(rawMessage: proto.IMessage | undefined): string | undefined {
  ...
  const labels = (contactsArray as ContactLike[])
    .map((entry: ContactLike) =>
      describeContact({ displayName: entry.displayName, vcard: entry.vcard }),
    )
    .map((entry: { name?: string; phones: string[] }) => formatContactLabel(entry.name, entry.phones))
...
export function describeReplyContext(rawMessage: proto.IMessage | undefined): {
  ...
  if (!body) {
    ...
    logVerbose(
      `Quoted message missing extractable body${quotedType ? ` (type ${String(quotedType)})` : ""}`,
    );
    return null;
  }
